...
func TestWindsurfImportScopedRules(t *testing.T) {
	dir := t.TempDir()
	os.MkdirAll(filepath.Join(dir, ".windsurf", "rules"), 0o755)

	// User-authored scoped rule with globs: frontmatter — should be imported.
	os.WriteFile(filepath.Join(dir, ".windsurf/rules/api.md"),
		[]byte("---\nglobs: **/*.go, internal/**\n---\n\n# API rules\nFollow REST conventions.\n"), 0o644)

	// agents.md should be skipped (no globs: field so treated as general rule, but
	// we also test that a generated agents.md is ignored).
	os.WriteFile(filepath.Join(dir, ".windsurf/rules/agents.md"),
		[]byte("> Generated by ajolote-ai from `.agents/config.json` — do not edit manually.\n"), 0o644)

	// A plain .md without globs: should not become a scoped rule.
	os.WriteFile(filepath.Join(dir, ".windsurf/rules/readme.md"),
		[]byte("# Notes\nSome general notes without globs.\n"), 0o644)
	...
	if len(result.NewScopedRules) != 1 {
		t.Fatalf("expected 1 scoped rule, got %d: %v", len(result.NewScopedRules), result.NewScopedRules)
	}
	sr := result.NewScopedRules[0]
	if sr.Name != "api" {
		t.Errorf("expected name 'api', got %q", sr.Name)
	}
	if len(sr.Globs) != 2 {
		t.Errorf("expected 2 globs, got %v", sr.Globs)
	}
	if sr.Path != ".agents/rules/api.md" {
		t.Errorf("unexpected path %q", sr.Path)
	}
	if body, ok := result.ScopedRuleContents["api"]; !ok || !strings.Contains(body, "API rules") {
		t.Errorf("scoped rule content missing or wrong: %q", body)
	}
}

func TestWindsurfImportCommands(t *testing.T) {
	dir := t.TempDir()
	os.MkdirAll(filepath.Join(dir, ".windsurf", "workflows"), 0o755)

	// User-authored workflow — should be imported as a command.
	os.WriteFile(filepath.Join(dir, ".windsurf/workflows/deploy.yaml"),
		[]byte("name: deploy\ndescription: Deploy the app\nsteps:\n  - name: Execute\n    say: |\n      Run deploy.sh\n      Check logs.\n"), 0o644)

	// ajolote-sync.yaml must be skipped.
	os.WriteFile(filepath.Join(dir, ".windsurf/workflows/ajolote-sync.yaml"),
		[]byte("name: ajolote-sync\ndescription: Sync config\nsteps:\n  - name: Run sync\n    run: ajolote sync windsurf\n"), 0o644)
	...
	if len(result.NewCommands) != 1 {
		t.Fatalf("expected 1 command, got %d: %v", len(result.NewCommands), result.NewCommands)
	}
	cmd := result.NewCommands[0]
	if cmd.Name != "deploy" {
		t.Errorf("expected name 'deploy', got %q", cmd.Name)
	}
	if cmd.Description != "Deploy the app" {
		t.Errorf("expected description 'Deploy the app', got %q", cmd.Description)
	}
	if !strings.Contains(cmd.Content, "Run deploy.sh") {
		t.Errorf("command content missing expected text, got: %q", cmd.Content)
	}
}

func TestCodexImportMCPServers(t *testing.T) {
	dir := t.TempDir()
	os.MkdirAll(filepath.Join(dir, ".codex"), 0o755)

	toml := `# Generated by ajolote-ai from .agents/config.json — do not edit manually
...
args = ["-y", "my-tool"]
`
	os.WriteFile(filepath.Join(dir, ".codex/config.toml"), []byte(toml), 0o644)
	...
	srv, ok := result.NewMCPServers["my-tool"]
	if !ok {
		t.Fatalf("expected my-tool in NewMCPServers, got: %v", result.NewMCPServers)
	}
	if srv.Command != "npx" {
		t.Errorf("expected command 'npx', got %q", srv.Command)
	}
	if len(srv.Args) != 2 || srv.Args[0] != "-y" || srv.Args[1] != "my-tool" {
		t.Errorf("unexpected args: %v", srv.Args)
	}
}

func TestCodexImportMCPServersWithEnv(t *testing.T) {
	dir := t.TempDir()
	os.MkdirAll(filepath.Join(dir, ".codex"), 0o755)
...
`
	...
	srv, ok := result.NewMCPServers["gh"]
	if !ok {
		t.Fatalf("expected 'gh' in NewMCPServers, got: %v", result.NewMCPServers)
	}
	if srv.Env["GITHUB_TOKEN"] != "ghp_secret" {
		t.Errorf("expected GITHUB_TOKEN='ghp_secret', got %q", srv.Env["GITHUB_TOKEN"])
	}
}
